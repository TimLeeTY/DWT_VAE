...
class VariationalAutoencoder(object):
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.relu,
        ...
        self.x = tf.placeholder(tf.float32, [None, network_architecture["n_output"].sum()])
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2,
        ...
        """
        all_weights = dict()
        if type(n_input).__module__ == np.__name__ and \
                type(n_hidden_recog_1).__module__ == np.__name__ \
                and type(n_hidden_gener_2).__module__ == np.__name__:
            """ If n_input is a list, the weight matrix in the 1st layer is initialised as a
            block diagonal such that there is no mixing between coefficients from different
            levels of wavelet decomposition
            """
            all_weights['weights_recog'] = {
                'h1': tf.concat([tf.concat([
                    tf.Variable(xavier_init(n_input[col], n_hidden_recog_1[row]))
                    if row == col else
                    tf.constant(0., shape=[n_input[col], n_hidden_recog_1[row]])
                    for col in range(len(n_input))], axis=0) for row in range(len(n_input))
                ], 1),
                'h2': tf.Variable(xavier_init(n_hidden_recog_1.sum(), n_hidden_recog_2)),
                'out_mean': tf.Variable(xavier_init(n_hidden_recog_2, n_z)),
                'out_log_sigma': tf.Variable(xavier_init(n_hidden_recog_2, n_z))}
            all_weights['biases_recog'] = {
                'b1': tf.Variable(tf.zeros([n_hidden_recog_1.sum()], dtype=tf.float32)),
            ...
            all_weights['weights_gener'] = {
                ...
                'h2': tf.Variable(xavier_init(n_hidden_gener_1, n_hidden_gener_2.sum())),
                'out_mean': tf.concat([tf.concat([
                    tf.Variable(xavier_init(n_hidden_gener_2[col], n_output[row]))
                    if row == col else
                    tf.constant(0., shape=[n_hidden_gener_2[col], n_output[row]])
                    for col in range(len(n_input))], axis=0) for row in range(len(n_input))
                ], 1)}
            all_weights['biases_gener'] = {
                'b1': tf.Variable(tf.zeros([n_hidden_gener_1], dtype=tf.float32)),
                'b2': tf.Variable(tf.zeros([n_hidden_gener_2.sum()], dtype=tf.float32)),
                'out_mean': tf.Variable(tf.zeros([n_output.sum()], dtype=tf.float32))}
            all_weights['batch_norm_gener'] = {
                'm1': tf.Variable(1, dtype=tf.float32),
                'b1': tf.Variable(0, dtype=tf.float32),
                'm2': tf.Variable(1, dtype=tf.float32),
                'b2': tf.Variable(0, dtype=tf.float32)}
        elif isinstance(n_input, float):
            """ if n_input is just a float, initialises 1st layer as a fully connected layer
            """
            all_weights['weights_recog'] = {
    ...
    def _recognition_network(self, weights, biases, batch_norm):
        """ Probabilistic encoder, maps input onto a Gaussian distribution in latent space
        ...
        layer_1 = \
            tf.nn.batch_normalization(tf.transpose(layer_1), layer_1_mean, layer_1_var,
        ...
        layer_2 = \
            tf.nn.batch_normalization(tf.transpose(layer_2), layer_2_mean, layer_2_var,
    ...
    def _generator_network(self, weights, biases, batch_norm):
        ...
        layer_1 = tf.nn.batch_normalization(
            tf.transpose(layer_1), layer_1_mean, layer_1_var,
            batch_norm['b1'], batch_norm['m1'], 1e-10)
        layer_1 = tf.transpose(self.transfer_fct(layer_1))
        layer_2 = tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])
        layer_2_mean, layer_2_var = \
            tf.reduce_mean(layer_2, axis=1), tf.math.reduce_variance(layer_2, axis=1)
        layer_2 = tf.nn.batch_normalization(
            tf.transpose(layer_2), layer_2_mean, layer_2_var,
            batch_norm['b2'], batch_norm['m2'], 1e-10)
        layer_2 = tf.transpose(self.transfer_fct(layer_2))
        x_reconstr_mean = \
    ...
    def _create_loss_optimizer(self, beta=0.1):
        """ Defines the cost functions assuming the data follows a multivariate Bernoulli
        Distribution: p(x) = x'^x * (1-x')^(1-x)
    ...
    def encode(self, X):
        """Encodes data by mapping it into the mean of the Gaussian in the latent space.
        """
...
def xavier_init(fan_in, fan_out, constant=0.5):
    ...
    maxval = constant*np.sqrt(6.0/(fan_in + fan_out))
    return tf.random_uniform((fan_in, fan_out), minval=-maxval, maxval=maxval,
                             dtype=tf.float32)


def train(dataset_iterator, network_architecture, learning_rate=0.001,
          batch_size=50, total_batch=400, training_epochs=5,
          display_step=1, valid=None, preproc='DWT'):
    ...
    for epoch in range(training_epochs):
        ...
        for iter in range(total_batch):
            ...
            # Create print string to display losses for each batch
            ...
            print(print_str)
        # Save variables to disk for use later after each epoch
    ...
    # Write cost logs out to be restored from disk
...
def test_plot(vae, valid, epoch, preproc='DWT'):
...
def pre_process(batch, preproc='DWT', wavelet='db1'):
...
def wavedec_coeff_len(wavelet_len, data_len, level):
    if level == 1:
        return [pywt.dwt_coeff_len(data_len, wavelet_len, mode='symmetric')]
    return ([pywt.dwt_coeff_len(
        wavedec_coeff_len(wavelet_len, data_len,
                          level-1)[0], wavelet_len, mode='symmetric')]
            + wavedec_coeff_len(wavelet_len, data_len, level-1))
...
def main():
    ...
    time_steps = 16000 * 4
    wavelet = 'db6'
    # Define features for parsing dataset
    nsynth_feature = {
    ...
    sub_samp = 2
    div = 8
    # Account for sub-sampling and truncation
    ...
    # Parse input data appropriately with labels
    nsynth_dataset = nsynth_dataset.map(_parse_input)

    # Apply filter for the distortion quality
    nsynth_dataset = nsynth_dataset.filter(filter_func)
    nsynth_dataset = nsynth_dataset.shuffle(buffer_size=10000).repeat().batch(batch_size)
    nsynth_iterator = nsynth_dataset.make_one_shot_iterator().get_next()['audio']

    # Repeat data-loading processes for valid dataset
    nsynth_valid = tf.data.TFRecordDataset("nsynth-valid.tfrecord")
    nsynth_valid = nsynth_valid.map(_parse_input)
    nsynth_valid = nsynth_valid.filter(filter_func)
    nsynth_valid = nsynth_valid.shuffle(buffer_size=1000).repeat().batch(batch_size)
    nsynth_valid_iterator = nsynth_valid.make_one_shot_iterator().get_next()['audio']

    # n_input initiated as a list of the lengths of the wavelet decomposition lengths
    # Will produce block diagonal weights in the first hidden layer
    # Can instead be a float for the default structure
    n_input = wavedec_coeff_len(2, 4000, 6)
    n_input = np.array([n_input[0]]+n_input)

    # Define number of nodes in each layer
    vae_architecture = dict(
        n_hidden_recog_1=n_input // 2,
        n_hidden_recog_2=1000,
        n_hidden_gener_1=1000,
        n_hidden_gener_2=n_input // 2,
        n_input=n_input,
        n_output=n_input,
        n_z=64
    )
    ...
    vae = train(nsynth_iterator, vae_architecture, training_epochs=40,
                valid=nsynth_valid_iterator, preproc='DWT', batch_size=batch_size)
    vae.sess.close()
